...
if __name__=="__main__":
    ...
    best_test_map = 0
    for gamma in gammas:
         test_maps = np.load("dro_passive/logs/maps_"+str(run)+"_"+str(gamma)+".npy")
         max_test_map = max(test_maps)
         if max_test_map>best_test_map:
             best_test_map = max_test_map
             best_gamma = gamma
         
    
    model_path = "dro_passive/Cifar100/models/model_"+str(run)+"_"+str(best_gamma)+"_best.pth.tar"
    ...
    np.save("Dataset/Cifar100/added_pos_data_"+str(run)+"_"+str(no_query)+"_admil_f_entropy.npy",queried_pos_data)    
    np.save("Dataset/Cifar100/added_neg_data_"+str(run)+"_"+str(no_query)+"_admil_f_entropy.npy", queried_neg_data)
