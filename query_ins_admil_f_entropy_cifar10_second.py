...
if __name__=="__main__":
    ...
    for gamma in gammas:
        for reg_coeff in reg_coeffs:
            maps = np.load("dro_active/Cifar10/logs/test_maps_"+str(run)+"_"+str(gamma)+"_"+str(prev_query)+"_"+str(reg_coeff)+"_admil_f_entropy.npy")
    ...
    prev_train_pos = np.load("Dataset/Cifar10/added_pos_data_"+str(run)+"_"+str(prev_query)+"_admil_f_entropy.npy.npy", allow_pickle = True)    
    prev_train_neg = np.load("Dataset/Cifar10/added_neg_data_"+str(run)+"_"+str(prev_query)+"_admil_f_entropy.npy.npy", allow_pickle = True)
        
    queried_instance_ids = getinstances(prev_train_pos, prev_train_neg)
    
    model_path = "dro_active/Cifar10/models/model_"+str(run)+"_"+str(best_gamma)+"_"+str(prev_query)+"_"+str(best_reg_coeff)+"_admil_f_entropy_best_map.pth.tar"
    ...
    np.save("Dataset/Cifar10/added_pos_data_"+str(run)+"_"+str(curr_query)+"_admil_f_entropy.npy",queried_pos_data)    
    np.save("Dataset/Cifar10/added_neg_data_"+str(run)+"_"+str(curr_query)+"_admil_f_entropy.npy", queried_neg_data)
