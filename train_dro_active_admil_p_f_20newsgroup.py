...
from process_input_20newsgroup import process_data
...
if __name__=="__main__":
    [_, run, topic_no, gamma, reg_coeff] = sys.argv
    ...
    lr = 0.1
    patience = 0
    mil_train_pos = np.load("Dataset/20NewsGroup/rep_"+str(run)+"_topic_"+topic+"_train_pos.npy", allow_pickle = True)
    
    mil_train_neg = np.load("Dataset/20NewsGroup/rep_"+str(run)+"_topic_"+topic+"_train_neg.npy", allow_pickle = True)

    lab_train_pos = np.load("Dataset/20NewsGroup/rep_"+str(run)+"_topic_"+topic+"_added_pos_"+str(no_query)+"_admil_p_f.npy", allow_pickle = True)
    lab_train_neg = np.load("Dataset/20NewsGroup/rep_"+str(run)+"_topic_"+topic+"_added_neg_"+str(no_query)+"_admil_p_f.npy", allow_pickle = True)
    mil_train_pos = process_data(mil_train_pos, lab_train_neg)
    
    if len(lab_train_pos)+len(lab_train_neg)==0:
        sys.exit()
    if no_query!=15:
        prev_lab_train_pos = np.load("Dataset/20NewsGroup/rep_"+str(run)+"_topic_"+topic+"_added_pos_"+str(no_query-15)+"_admil_p_f.npy", allow_pickle = True)
        prev_lab_train_neg = np.load("Dataset/20NewsGroup/rep_"+str(run)+"_topic_"+topic+"_added_neg_"+str(no_query-15)+"_admil_p_f.npy", allow_pickle = True)
        if len(prev_lab_train_pos)+len(prev_lab_train_neg)==len(lab_train_pos)+len(lab_train_neg):
            sys.exit()
        
        
    lab_feats, lab_labels, _ = process_labeled_ins(lab_train_pos, lab_train_neg)

    test_pos = np.load("Dataset/20NewsGroup/rep_"+str(1)+"_topic_"+topic+"_test_pos.npy", allow_pickle = True)
    test_neg = np.load("Dataset/20NewsGroup/rep_"+str(1)+"_topic_"+topic+"_test_neg.npy", allow_pickle = True)
    ...
    for i in range(max_iterations):
        ...
        if i%10==0:
            ...
            if test_ap>best_test_map:
                ...
                'opt_dict': optimizer.state_dict(),}, os.path.join("dro_active/20NewsGroup/models/model_"+str(run)+"_"+str(gamma)+"_"+str(topic)+"_"+str(no_query)+"_"+str(reg_coeff)+\
    ...
    np.save("dro_active/20NewsGroup/logs/test_aucs_"+str(run)+"_"+str(gamma)+"_"+str(topic)+"_"+str(no_query)+"_"+str(reg_coeff)+"_admil_p_f.npy", test_aucs)
    np.save("dro_active/20NewsGroup/logs/test_maps_"+str(run)+"_"+str(gamma)+"_"+str(topic)+"_"+str(no_query)+"_"+str(reg_coeff)+"_admil_p_f.npy", test_maps)
    np.save("dro_active/20NewsGroup/logs/dro_losses_"+str(run)+"_"+str(gamma)+"_"+str(topic)+"_"+str(no_query)+"_"+str(reg_coeff)+"_admil_p_f.npy", dro_losses)
    np.save("dro_active/20NewsGroup/logs/reg_losses_"+str(run)+"_"+str(gamma)+"_"+str(topic)+"_"+str(no_query)+"_"+str(reg_coeff)+"_admil_p_f.npy", reg_losses)
